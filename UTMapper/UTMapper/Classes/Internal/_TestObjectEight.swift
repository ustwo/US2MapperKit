	...
	var non_optionalDictionaryType : Dictionary<String,TestObjectFour>
 	...
 	convenience init?(_ dictionary: Dictionary<String, AnyObject>) {
 		...
 		if let valuesDict = UTMapper.parseJSONResponse(className!, data : dictionary) {
			...
			let temp_non_optionalDictionaryType : Dictionary<String, TestObjectFour> = UTMapper.typeCast(valuesDict["non_optionalDictionaryType"])!

			if let unwrapped_optionalDictionaryType = valuesDict["optionalDictionaryType"] {
 				temp_optionalDictionaryType = UTMapper.typeCast(unwrapped_optionalDictionaryType)
 			}
