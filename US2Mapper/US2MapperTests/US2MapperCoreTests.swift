//
//  US2MapperCoreTests.swift
//  US2Mapper
//
//  Created by Anton on 7/7/15.
...
class US2MapperCoreTests: XCTestCase {
    ...
    func test_retrieveMappingConfiguration() {
        ...
        if let propertyMapping = mapping["optionalSubType"] {
            if let optionalSubtypeMapping = propertyMapping["optionalSubType"] {
                XCTAssertEqual(optionalSubtypeMapping["key"], "optional_subtype", "retrieveMappingConfiguration() did not retrieve the correct mapping")
                XCTAssertEqual(optionalSubtypeMapping["type"], "TestObjectThree", "retrieveMappingConfiguration() did not retrieve the correct mapping")
            }
            
            if let nonoptionalSubtypeMapping = propertyMapping["non_optionalSubType"] {
                XCTAssertEqual(nonoptionalSubtypeMapping["key"], "non_optional_subtype", "retrieveMappingConfiguration() did not retrieve the correct mapping")
                XCTAssertEqual(nonoptionalSubtypeMapping["type"], "TestObjectThree", "retrieveMappingConfiguration() did not retrieve the correct mapping")
                XCTAssertEqual(nonoptionalSubtypeMapping["nonoptional"], "true", "retrieveMappingConfiguration() did not retrieve the correct mapping")
            }
        }
    }
    ...
    func test_parsedDictionaryOfNativeValuesArray() {
        ...
        for (_, stringValue) in returnedStringDictionary {
            XCTAssertTrue(stringArray.contains(stringValue as! String), "String values parsed incorrectly - Failing Method parsedDictionaryOfNativeValuesArray()")
        }
        
        XCTAssertEqual(returnedStringDictionary.keys.count, stringArray.count, "Missing String values - Failing Method parsedDictionaryOfNativeValuesArray()")
        
        for (_, integerVal) in returnedIntDictionary {
            XCTAssertTrue(intArray.contains(integerVal as! Int), "Int values parsed incorrectly - Failing Method parsedDictionaryOfNativeValuesArray()")
        }
    
        XCTAssertEqual(returnedIntDictionary.keys.count, intArray.count, "Missing Int values - Failing Method parsedDictionaryOfNativeValuesArray()")
        
        for (_, doubleValue) in returnedDoubleDictionary {
            XCTAssertTrue(doubleArray.contains(doubleValue as! Double), "Double values parsed incorrectly - Failing Method parsedDictionaryOfNativeValuesArray()")
        }
        
        XCTAssertEqual(returnedDoubleDictionary.keys.count, doubleArray.count, "Missing Double values - Failing Method parsedDictionaryOfNativeValuesArray()")
        
        for (_, floatVal) in returnedFloatDictionary {
            XCTAssertTrue(floatArray.contains(floatVal as! Float), "Float values parsed incorrectly - Failing Method parsedDictionaryOfNativeValuesArray()")
        }
        
        XCTAssertEqual(returnedFloatDictionary.keys.count, floatArray.count, "Missing Float values - Failing Method parsedDictionaryOfNativeValuesArray()")
        ...
        XCTAssertEqual(returnedBoolDictionary.keys.count, boolArray.count, "Missing Bool values - Failing Method parsedDictionaryOfNativeValuesArray()")
    }
    ...
    func test_parsedArrayOfNativeValuesArray() {
        ...
        for stringValue in returnedStringArray {
            XCTAssertTrue(stringArray.contains(stringValue as! String), "String values parsed incorrectly - Failing Method parsedArrayOfNativeValuesArray()")
        }
        ...
        for integerVal in returnedIntArray {
            XCTAssertTrue(intArray.contains(integerVal as! Int), "Int values parsed incorrectly - Failing Method parsedArrayOfNativeValuesArray()")
        }
        
        XCTAssertEqual(returnedIntArray.count, intArray.count, "Missing Int values - Failing Method parsedArrayOfNativeValuesArray()")
        
        for doubleValue in returnedDoubleArray {
            XCTAssertTrue(doubleArray.contains(doubleValue as! Double), "Double values parsed incorrectly - Failing Method parsedArrayOfNativeValuesArray()")
        }
        
        XCTAssertEqual(returnedDoubleArray.count, doubleArray.count, "Missing Double values - Failing Method parsedArrayOfNativeValuesArray()")
        
        for floatVal in returnedFloatArray {
            XCTAssertTrue(floatArray.contains(floatVal as! Float), "Float values parsed incorrectly - Failing Method parsedArrayOfNativeValuesArray()")
        }
        
        XCTAssertEqual(returnedFloatArray.count, floatArray.count, "Missing Float values - Failing Method parsedArrayOfNativeValuesArray()")
        ...
        XCTAssertEqual(returnedBoolArray.count, boolArray.count, "Missing Bool values - Failing Method parsedArrayOfNativeValuesArray()")
    }
    ...
    func test_parsedArrayOfNativeValuesDictionary() {
        ...
        for (_, stringValue) in stringDictionary {
            XCTAssertTrue(returnedStringArray.contains(stringValue), "String values parsed incorrectly - Failing Method parsedArrayOfNativeValuesDictionary()")
        }
        
        XCTAssertEqual(returnedStringArray.count, stringDictionary.keys.count, "Missing String values -  Failing Method parsedArrayOfNativeValuesDictionary()")
        
    
        for (_, intVal) in intDictionary {
            XCTAssertTrue(returnedIntArray.contains(intVal), "Int values parsed incorrectly - Failing Method parsedArrayOfNativeValuesDictionary()")
        }
        
        XCTAssertEqual(returnedIntArray.count, intDictionary.keys.count, "Missing Int values -  Failing Method parsedArrayOfNativeValuesDictionary()")
        
        
        for (_, doubleVal) in doubleDictionary {
            XCTAssertTrue(returnedDoubleArray.contains(doubleVal), "Double values parsed incorrectly - Failing Method parsedArrayOfNativeValuesDictionary()")
        }
        
        XCTAssertEqual(returnedDoubleArray.count, doubleDictionary.keys.count, "Missing Double values -  Failing Method parsedArrayOfNativeValuesDictionary()")
        
        
        for (_, floatValue) in floatDictionary {
            XCTAssertTrue(returnedFloatArray.contains(floatValue), "Float values parsed incorrectly - Failing Method parsedArrayOfNativeValuesDictionary() ")
        }
        
        XCTAssertEqual(returnedFloatArray.count, floatDictionary.keys.count, "Missing Float values -  Failing Method parsedArrayOfNativeValuesDictionary()")
        ...
        XCTAssertEqual(returnedBoolArray.count, boolDictionary.keys.count, "Missing Bool values -  Failing Method parsedArrayOfNativeValuesDictionary()")
    }
